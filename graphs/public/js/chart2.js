...
getData(loadTestMap, function (err, data) {
  ...
  graph({title: `Process Count`, width: 1200, height: 300}, 
    Object.keys(loadTestMap).map(id => {
      return {
        id: id.toLowerCase(),
        values: data[id].map((d, idx) => {
          return {step: idx, val: d.count}
        })
      }
    })
  )
  ...
  'dns99Ptile', 'gcCpuPercent', 'gcDurUsMedian', 'gcDurUs99Ptile',
  'httpClientMedian', 'httpClient99Ptile', 'httpServerMedian', 'httpServer99Ptile',
  ...
  keyMetrics.forEach(metric => {
    compareMetricGraph(metric, data)
  })
...
function genStatGraph (test, metric, data) {
  ...
  let metrics = [`${metric}Min`, `${metric}Mean`, `${metric}`, `${metric}Max`].map(id => {
    return {
      id: id.toLowerCase(),
...
function compareMetricGraph (metric, data) {
  ['Min', 'Max', 'Mean', ''].forEach(type => {
    let metrics = ['cpu', 'capacity'].map(id => {
      return {
        id: id.toLowerCase(),
        values: data[id].map((d, idx) => {
          return {step: idx, val: d[metric+type]}
        })
      }
    })
    if (!type) type = 'Avg'
    graph({title: `${metric}-${type} Compare`, width: 400}, metrics)
  })
}
